...
class SkipGramNeg(nn.Module):
    ...
    def __init__(self, n_vocab: int, n_embed: int, noise_dist: torch.Tensor = None):
        ...
        self.in_embed: nn.Embedding = nn.Embedding(
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(
        ...
        out_dot_product = torch.bmm(
            input_vectors.unsqueeze(1), output_vectors.unsqueeze(2)
        ).squeeze()
        out_loss = torch.nn.functional.logsigmoid(out_dot_product)

        # Compute log-sigmoid loss for incorrect classifications
        noise_dot_product = torch.bmm(
            noise_vectors, input_vectors.unsqueeze(2)
        ).squeeze(2)
